...
- Добавить проверку типа прошивки (UEFI vs BIOS) и ветвления логики.
- Бэкап `/etc/default/grub` и `/etc/grub.d` перед изменениями.
- Поддержка локалей/меню выбора операций.
- Тесты и CI для безопасных правок.
